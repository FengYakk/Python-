...
class Transmit:
 ...
 def processRecv(self):
  ...
  infile=open("recv3.txt","wb")
  infile.write(data)
  infile.close()
  tcpCliSock.send('success'.encode())
  tcpCliSock.close()
  self.text.insert(INSERT,'>接收到加密文件并存入recv3.txt\n')

 def processDecrypt(self):
  outfile=open("recv3.txt","rb")
  list=[]
  while True:
   a=outfile.read(1)
   if not a:break
...
class Analyze:
 ...
 def processPic5(self):
  ...
  for i in range(count):
   ...
   size=int(size)
   data = tcpCliSock.recv(size)
   myfile = open('客户端3词云'+str(i)+'.jpg', 'wb')
   myfile.write(data)
   myfile.close()
   tcpCliSock.send('OK'.encode())
  for i in range(count):
   img = cv2.imdecode(np.fromfile('客户端3词云'+str(i)+'.jpg',dtype=np.uint8),-1)
   cv2.imshow("Image", img)
   cv2.waitKey (0)
  tcpCliSock.close()
