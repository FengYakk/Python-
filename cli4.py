...
class Transmit:
 def __init__(self):
  ...
  Button(frame2,text="转发至客户端2",command=self.processTrans2).grid(row=1,column=2)
  Button(frame2,text="转发至客户端3",command=self.processTrans3).grid(row=1,column=3)
  frame3=Frame(window)
  frame3.pack()
 ...
 def processRecv(self):
  ...
  infile=open("recv4.txt","wb")
  infile.write(data)
  infile.close()
  tcpCliSock.send('success'.encode())
  tcpCliSock.close()
  self.text.insert(INSERT,'>接收到加密文件并存入recv4.txt\n')

 def processDecrypt(self):
  outfile=open("recv4.txt","rb")
  list=[]
  while True:
   a=outfile.read(1)
   if not a:break
...
class Analyze:
 ...
 def processPic5(self):
  ...
  for i in range(count):
   ...
   size=int(size)
   data = tcpCliSock.recv(size)
   myfile = open('客户端4词云'+str(i)+'.jpg', 'wb')
   myfile.write(data)
   myfile.close()
   tcpCliSock.send('OK'.encode())
  for i in range(count):
   img = cv2.imdecode(np.fromfile('客户端4词云'+str(i)+'.jpg',dtype=np.uint8),-1)
   cv2.imshow("Image", img)
   cv2.waitKey (0)
  tcpCliSock.close()
