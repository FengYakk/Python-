...
class Transmit:
 def __init__(self):
  ...
  frame2=Frame(window)
  frame2.pack()
  Button(frame2,text="转发至客户端2",command=self.processTrans2).grid(row=1,column=1)
 ...
 def processEncrypt(self,e):
  ...
  self.text.insert(INSERT,'\n')
  f.close()
  outfile=open(filename,"rb",encoding='utf-8')
  list=[]
  while True:
   a=outfile.read(1)
   if not a:break
 ...
 def processTrans2(self):
  outfile=open("jiami.txt","rb")
  tcpCliSock = socket(AF_INET, SOCK_STREAM)
  tcpCliSock.connect(ADDR)
  tcpCliSock.send('加密文件转发mode2'.encode())
  data = tcpCliSock.recv(BUFSIZ).decode()
  data=outfile.read()
  if not data:
   return
 ...
 def processRecv(self):
  ...
  infile=open("recv1.txt","wb")
  infile.write(data)
  infile.close()
  tcpCliSock.send('success'.encode())
  tcpCliSock.close()
  self.text.insert(INSERT,'>接收到加密文件并存入recv1.txt\n')

 def processDecrypt(self):
  outfile=open("recv1.txt","rb")
  list=[]
  while True:
   a=outfile.read(1)
   if not a:break
...
class Detection:
 ...
 def processResult(self):
  ...
  if rec=='No face':
   ...
   canvas.pack()
   canvas.create_text(300,100,text=''.join(['性别：',list[0]]),font=('Times',40))
   canvas.create_text(300,300,text=''.join(['预测年龄：',list[1]]),font=('Times',40))
   canvas.create_text(300,500,text=''.join(['颜值得分：',list[2]]),font=('Times',40))
...
class Analyze:
 ...
 def processPic5(self):
  ...
  for i in range(count):
   ...
   size=int(size)
   data = tcpCliSock.recv(size)
   myfile = open('客户端1词云'+str(i)+'.jpg', 'wb')
   myfile.write(data)
   myfile.close()
   tcpCliSock.send('OK'.encode())
  for i in range(count):
   img = cv2.imdecode(np.fromfile('客户端1词云'+str(i)+'.jpg',dtype=np.uint8),-1)
   cv2.imshow("Image", img)
   cv2.waitKey (0)
  tcpCliSock.close()
...
class Cli:
 def __init__(self):
  ...
  Button(frame1,text="目标识别",command=self.processRecognize).grid(row=1,column=5)
  window.mainloop()
